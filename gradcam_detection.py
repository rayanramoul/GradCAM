...
def eval_voc_grad(visualise=False, threshold=0.65):
    ...
    metadata = read_metadata(dataset_test)
    all_gt_dogs = produce_gt_dog_boxes(metadata)
    ...
    """
    grad_cam = GradCam(model, feature_layer=model.layer4, target_layer=['2'], use_cuda=False)

    all_detected_dogs = {}
    print("Predicting")
    for key, value in tqdm(metadata.items()):

         # Looking if there is a dog in this objects of this image
        
        objects = value['xml_parsed']['annotation']['object']
        object_names = [o['name'] for o in objects]
        if 'dog' not in object_names:
            continue
        """
        if key not in all_gt_dogs.keys():
            continue
        """
        # Get path of the file from dataset
        image_path = key
        dogs = []
        imname = metadata[key]['imname']
        ...
        input_img, grayscale_cam, cam, img = eval_image(model, grad_cam, image_path, target_category=4)
        ...
        for obj in objects[::-1]:
            ...
            dogs.append(np.r_[bbox, 4.0])
        
        dogs = np.array(dogs)
        
        all_detected_dogs[imname] = dogs
        ...
        if visualise:
            ...
            # Loop in each object of ground truth
            for obj in metadata[key]['xml_parsed']['annotation']['object']:
                if obj['name'] != 'dog':
                    continue
                gt = obj['bndbox']
                xmingt, xmaxgt, ymingt, ymaxgt = int(gt['xmin']), int(gt['xmax']), int(gt['ymin']), int(gt['ymax'])
    ...
    with open('results/all_detected_dogs'+str(threshold)+'.pickle', 'wb') as handle:
        pickle.dump(all_detected_dogs, handle, protocol=pickle.HIGHEST_PROTOCOL)

    with open('results/all_gt_dogs.pickle', 'wb') as handle:
        pickle.dump(all_gt_dogs, handle, protocol=pickle.HIGHEST_PROTOCOL)
    stats_df = eval_stats_at_threshold(
            all_detected_dogs, all_gt_dogs)
    log.info('GradCAM dogs:\n{}'.format(stats_df))


def  load_eval_gradcam(threshold=0.05):
    """
    Load existing pickle files for predicted bounding boxes and ground truth ones
    and evaluate them ( for faster debugging )
    """
...
if __name__ == '__main__':
    ...
    for tr in thresholds:
        eval_voc_grad(visualise=True, threshold=tr)